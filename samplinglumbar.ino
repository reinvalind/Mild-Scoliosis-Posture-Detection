...
#include <Wire.h> 
// #include <math.h> // <-- Tidak diperlukan lagi
...
WiFiServer server(8002); // Server TCP untuk Lumbar, Port 8002
WiFiClient client;
...
#define factor -1 // -1 FOR LOWER BACK MCU
...
void calculate_angles() {
    ...
    read_imu_data(MPU_addr_2, y_2, z_2); 
    T_sagital = convertToThreeDegreeStep((int)(factor * (-y_1 + y_2))); 
    T_coronal = convertToThreeDegreeStep((int)z_2);
    if(T_coronal > 180){ 
...
void calibration_routine() {
    ...
    if ((y_1 > 100)) { 
        atas_siap = false;
        atas_status = "ATAS SALAH (ERROR: " + String(100.0 - y_1, 2) + ")";
    } else if (y_1 < 80) {
        atas_siap = false;
        atas_status = "ATAS SALAH (ERROR: " + String(80.0 - y_1, 2) + ")";
    } 
    ...
    if ((y_2 > 90)) { 
        bawah_siap = false;
        bawah_status = "BAWAH SALAH (ERROR: " + String(90.0 - y_2, 2) + ")";
    } else if (y_2 < 69) {
        bawah_siap = false;
        bawah_status = "BAWAH SALAH (ERROR: " + String(69.0 - y_2, 2) + ")";
    } 
    ...
    // Kirim SIAP_REFERENSI hanya jika KEDUANYA siap
...
void setup() { 
    ...
    if (WiFi.status() == WL_CONNECTED) {
        Serial.println("\nWiFi Terhubung.");
        Serial.print("IP Address Server Lumbar: ");
    ...
    Serial.println("Lumbar Server Siap.");
} 
    ...
    if (is_connected && client.connected()) {
        ...
        if (sampling_active) {
            ...
            if (current_time - last_send_time >= interval) {
                ...
                String data_str = "L:" + String(diff_sagital, 2) + "," + String(diff_coronal, 2);
                
                send_data(data_str.c_str());
                Serial.println("Sent to Python: " + data_str);

                // --- BLOK LOGIKA ALARM (digitalWrite) DIHAPUS ---
                // Tidak ada lagi pengecekan THRESHOLD atau fabs()
                // --- AKHIR PENGHAPUSAN ---
            }
        }
